...
How many variations of UI should be shown to different participants in the experiment?

Two. Because we just need to estimate two different apps.



How to let each user test all variations of the UI?

Need to control variables. 

For each user, different UI versions should be tested at different times. 

For different users, the testing environment, devices, and background knowledge of the software should be kept as consistent as possible.

How long would it take for the participant?

It depends on the the complexity of the task, but I think around 30 min is a good choice, or the user may feel impatient which will make the result unreliable.
