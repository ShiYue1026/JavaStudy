...
- 客户端会继续重传，直到次数限制
- 服务端此时会阻塞在accept()处，等待客户端发送ACK报文
...
**2. 防止当前ip+端口i已失效的连接请求报文段出现在本连接中**
...
- 表示 TCP 建立连接完成后，一开始不要发送大量的数据，而是先探测一下网络的拥塞程度。由小到大逐渐增加拥塞窗口的大小，如果没有出现丢包，**每收到一个 ACK，就将拥塞窗口 cwnd 大小就加 1（单位是 MSS）**。**每轮次**发送窗口增加一倍，呈指数增长，如果出现丢包，拥塞窗口就减半，进入拥塞避免阶段。
