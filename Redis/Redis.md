...
- 只考虑”最近访问“，而不考虑访问频率
  - 一个刚被访问过一次的数据可能比常用但刚好一段时间没用的数据保留得更久 → 导致热点数据被误淘汰
- 对周期性访问场景不友好
...
- 难以应对”新热点“
  - 新加入的数据访问频率初始为1，很难赶超老数据，新热点迟迟进不来

- 高频短时访问会被长时间记住
  - 某个数据被刷频后，即使后面没人访问，依旧留在缓存 → 滞留问题
...
一般单个key超过10kb就被认为是大key，会导致网络延迟增大、阻塞Redis性能、内存不足OOM等问题。
...
big hash：可以将数据分段存储，假设存了100万的用户数据，可以拆分成200个key，每个key下面存放5000个用户数据



**2. 压缩数据**

在存储前对较大的数据进行压缩，从而减少存储占用空间。
...
(Decide 4.NumberOut False/Base 2.NumberOut) > 0.02
